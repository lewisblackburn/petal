...
FOR EACH ROW
BEGIN
    -- Insert a new record into the FilmVersion model
...
BEGIN
    ...
    VALUES (
      ...
      CURRENT_TIMESTAMP,
      json_object(
          'title', CASE WHEN NEW.title != OLD.title THEN OLD.title ELSE "" END,
          'overview', CASE WHEN NEW.overview != OLD.overview THEN OLD.overview ELSE "" END
      ),
      json_object(
          'title', CASE WHEN NEW.title != OLD.title THEN NEW.title ELSE "" END,
          'overview', CASE WHEN NEW.overview != OLD.overview THEN NEW.overview ELSE "" END
      )
    );
END;
